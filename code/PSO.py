...
robot12 = turtle.Turtle()
robot12.shape("turtle")
robot12.color("red")
robot12.penup()
robot12.goto(-300, 50)

robot13 = turtle.Turtle()
robot13.shape("turtle")
robot13.color("red")
robot13.penup()
robot13.goto(-300, -50)
...
robot22 = turtle.Turtle()
robot22.shape("turtle")
robot22.color("blue")
robot22.penup()
robot22.goto(300, 150)

robot23 = turtle.Turtle()
robot23.shape("turtle")
robot23.color("blue")
robot23.penup()
robot23.goto(300, -150)
...
def robot1_auto_move(direction):
    ...
    if direction == "up":
        ...
        if robot1.xcor() + 10 <= 450:
            robot1.setx(robot1.xcor() + 10)
def robot12_auto_move(direction):
    # 随机移动
    direction = random.choice(["up", "down", "left", "right"])
    if direction == "up":
        if robot12.ycor() + 10 <= 300:
            robot12.sety(robot12.ycor() + 10)
    elif direction == "down":
        if robot12.ycor() - 10 >= -300:
            robot12.sety(robot12.ycor() - 10)
    elif direction == "left":
        if robot12.xcor() - 10 >= -450:
            robot12.setx(robot12.xcor() - 10)
    elif direction == "right":
        if robot12.xcor() + 10 <= 450:
            robot12.setx(robot12.xcor() + 10)
def robot13_auto_move(direction):
    # 随机移动
    direction = random.choice(["up", "down", "left", "right"])
    if direction == "up":
        if robot13.ycor() + 10 <= 300:
            robot13.sety(robot13.ycor() + 10)
    elif direction == "down":
        if robot13.ycor() - 10 >= -300:
            robot13.sety(robot13.ycor() - 10)
    elif direction == "left":
        if robot13.xcor() - 10 >= -450:
            robot13.setx(robot13.xcor() - 10)
    elif direction == "right":
        if robot13.xcor() + 10 <= 450:
            robot13.setx(robot13.xcor() + 10)
...
def robot22_auto_move():
    # 随机移动
    robot2 = robot22
    direction = random.choice(["up", "down", "left", "right",'0'])
    if direction == "up":
        if robot2.ycor() + 10 <= 300:
            robot2.sety(robot2.ycor() + 10)
    elif direction == "down":
...
for i in range(MAX_EPISODES):
    s1 = list(robot1.pos())+list(ball.pos())+list(robot2.pos())
    s2 = list(robot12.pos()) + list(ball.pos()) + list(robot22.pos())
    s3 = list(robot13.pos()) + list(ball.pos()) + list(robot23.pos())
    # print(s)
    ep_reward = 0
    for j in range(20):
        while True:
            ...
            robot1_auto_move(direction[dic1])

            a2 = ddpg.choose_action(s2)
            dic2 = np.argmax(a2)
            a2 = np.clip(np.random.normal(a2, var), -2, 2)  # add randomness to action selection for exploration
            robot12_auto_move(direction[dic2])

            a3 = ddpg.choose_action(s3)
            dic3 = np.argmax(a3)
            a3 = np.clip(np.random.normal(a3, var), -2, 2)  # add randomness to action selection for exploration
            robot13_auto_move(direction[dic3])
            ...
            robot23_attack()
            window.update()
            s_1 = list(robot1.pos()) + list(ball.pos()) + list(robot2.pos())
            s1 = s_1
            s_2 = list(robot12.pos()) + list(ball.pos()) + list(robot22.pos())
            s2 = s_2
            s_3 = list(robot13.pos()) + list(ball.pos()) + list(robot23.pos())
            s3 = s_3
